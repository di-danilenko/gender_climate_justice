...
data <- data %>%
  select(c("id", "majority_female_binary", "abstract","title","keywords",
...
```{r}
# prepare data to compile a summary statistics table

sumvarsFac <- c("first_author_female",
                "last_author_female",
                "broader_subfield",
                "impact",
                "X1_gii_quartile",
                "X1_region",
                "X2_region",
                "geoparsed_region", # keep only the first region, or assign category "multiple"
                "X2_gii_quartile")

sumvarsNum <- c("year",
                "proportion_female")

data[sumvarsFac] <- lapply(data[sumvarsFac], factor)
data[sumvarsNum] <- lapply(data[sumvarsNum], as.numeric)

sumVars <- c(sumvarsFac, sumvarsNum)
```

```{r}
# compile a summary statistics table
library(haven)
library(writexl)
library(openxlsx)
library(dplyr)
...
summary(data$proportion_female)
