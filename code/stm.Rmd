...
data <- data %>%
  mutate(impact_quartile = if_else(is.na(impact_quartile) | impact_quartile == "None", "unknown", impact_quartile))
...
# it seems to be running if we get rid of spline - it probably also runs on continuous variables then - but how do we feed them as categorical variables to stm while preserving the original data - also through spline? how do we find a reasonable number of splines?
# effects of female are a bit higher if we have year as an integer variable here

# try to run this with continuous data for the same variables

# try to run the linear regression outside - GII and impact as continuous variables
# from the stm output - could we pull the topic score uncertainty? / distribution? of each of the scores

# try to halve the number of categories for subfields - least priority, last resort

# gender, spline of year, subfield, gii quartile and impact by themselves are not causing the warning
# the interaction with subfield is causing the warning - too many factor levels
...
kmeans_model <- kmeans(tsne[c("comp.1","comp.2")], centers = 150)
...
output_file <- "/Users/dianadanilenko/Desktop/gender_climate_justice/code/graphs/tsne.png"
width <- 11
height <- 8
resolution <- 1000

# try to get a close up snapshot of a cluster with CJ topics
...
plot <- tsne %>% ggplot(aes(x = comp.1, y = comp.2)) +
  ...
  theme(aspect.ratio=1,
        plot.title = element_text(hjust = 0.5, size = 20),
        plot.subtitle = element_text(hjust = 0.5)) + theme(text = element_text(size = 15)) +
  scale_color_manual(values=c (wes_palette("Darjeeling1")[3] ,wes_palette("Darjeeling1")[4],wes_palette("Darjeeling1")[2], "darkgrey")) + guides(color=guide_legend(order = 1, override.aes=list(size=2.5)), alpha=guide_legend(order = 0, override.aes=list(size=2.5))) + 
      annotate("rect", xmin = x_min, xmax = x_max, ymin = y_min, ymax = y_max, 
           color = "black", fill = NA, linewidth = 0.5)
...
tsne_zoom <- tsne %>%
  filter(comp.1 > 15 & comp.2 > -15 & comp.2 < 30)
...
# to make this readable we need to filter the label data and add labels and add a rectangle to the main plot outlining where this belongs
...
plot <- tsne_zoom %>% ggplot(aes(x = comp.1, y = comp.2)) +
  ...
  #xlim(16,53) + ylim(-15,30) +
     scale_x_continuous(limits = c(16, 55), expand = c(0, 0)) +
...
plot <- tsne_zoom %>% ggplot(aes(x = comp.1, y = comp.2)) +
  ...
  theme(aspect.ratio=1,
        plot.title = element_text(hjust = 0.5, size = 20),
        plot.subtitle = element_text(hjust = 0.5)) + theme(text = element_text(size = 15)) +
  scale_color_manual(values=c (wes_palette("Darjeeling1")[3],wes_palette("Darjeeling1")[4],wes_palette("Darjeeling1")[2],"darkgrey")) + guides(color = guide_legend(order = 1,override.aes = list(size = 2.5))) + guides(alpha = guide_legend(order = 0,override.aes = list(size = 2.5)))
...
# this will omit topic score uncertainty
...
```{r}
### EXTERNAL REGRESSION ANALYSIS ### 
# do the same thing but by topic - each one separately and all the input variables that are originally continuous need to be continuous here
# instead of using mean proportions here i need the DTM but only for the relevant topics

new_col_names <- as.character(1:120)
colnames(DTM) <- new_col_names

# filter by cj_topics
# we still need to pull the impact as a continuous variable 
# we still need to pull the GII as a continuous variable
```
# heatmap
```{r}
### create a heatmap - CJ topic x continent

# this chunk of code is redundant
# what does this code do?
dplyr::count(data_complete, X1_country, sort = TRUE)
dplyr::count(data_complete, X1_region, sort = TRUE)
...
data <- merge(data, tsne[c('id', 'topic',"climatejusticescore")], by='id')
...
# Max: we could calculate % of overall documents from this region instead - i'm not particularly sure how this would improve/change the visual
...
```{r}
df_journal <- read.csv("data/df_journal.csv")

# so this still needs to be run for journal instead of publisher - in py - and the data updated

# for some reason journal_impact and quartile are entirely empty here
```
...
result <- consolidated_data %>%
  count(X1_region, Topic)/
# add divison by overall count of documents per region - where do i have this data?
result <- result %>%
  rename("Documents" = n)

merged_data <- merge(result, region_counts, by.x = "X1_region", by.y = "Var1", all.x = TRUE)

merged_data$relative <- merged_data$Documents/merged_data$Freq
...
mat4 <- mat4 %>%
  rename(topic = Topic)
mat4 <- merge(mat4,topic_names[c("topic","topicname")],by='topic')
rownames(mat4) <- mat4$topicname
mat4 <- mat4[-1]
mat4 <- mat4[-9]

mat4$World <- rowSums(mat4, na.rm = TRUE)

# these get a different order as well
...
```{r}
# this is looking really dumb maybe this doesnt make sense
...
data1 <- read_csv("data/data_complete.csv")
...
data1 <- data1 %>%
  mutate(
    ...
    majority_female_binary = as.numeric(majority_female_binary),
    climatejusticescore = as.numeric(climatejusticescore),
    year = as.numeric(year)
  )

# you want to use majority female not majority female binary for this!
...
summary_data1 <- data1 %>%
  ...
  summarize(
    ...
    avg_majority_female_binary = sum(majority_female_binary, na.rm = TRUE) / sum(!is.na(majority_female_binary))
  )
...
summary_data$avg_female[65:128] <- summary_data$avg_majority_female_binary[65:128]
...
ggsave("femaleauthorsharetime.png", width=10, height=8)

# this is kind of a boring graph let's see what would be nice to add here - a stacked percentised version next to it? 
...
summary_data2 <- data1 %>%
  group_by(year) %>%
  summarize(
    total_climatejusticescore = sum(climatejusticescore),
    resilience = sum(resilience),
    food_security = sum(food_security),
    vulnerability = sum(vulnerability),
    adaptive_capacity = sum(adaptive_capacity),
    governance = sum(governance),
    local_knowledge = sum(local_knowledge),
    island_territories = sum(island_territories),
    climate_finance = sum(climate_finance),
    rural_households = sum(rural_households),
    transformation_discourse = sum(transformation_discourse),
    community = sum(community),
    social_capital = sum(social_capital),
...
ggplot(summary_data2_long %>% filter(! topic == "total_climatejusticescore"), aes(x = year, y = value, fill = topic)) +
...
# exclude total cj score from this - add it as a line instead
...
# gender variables - better factor or better numeric?
# subfield variable has too many levels - exclude or recode into broader fields? does it make sense?

# how is the estimate for intercept 57 if the variable values are 0-1?
...
for (topic in cj_topicnames1) {
  model1 <- betareg(topic ~first_author_female + year + #subfield 
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
  summary(model1)  
}
...
model1_reg <- betareg(resilience ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg2 <- betareg(food_security ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg3 <- betareg(vulnerability ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg4 <- betareg(adaptive_capacity ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg5 <- betareg(governance ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg6 <- betareg(local_knowledge ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg7 <- betareg(island_territories ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg8 <- betareg(climate_finance ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg9 <- betareg(rural_households ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg10 <- betareg(transformation_discourse ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg11 <- betareg(community ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg12 <- betareg(social_capital ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model1_reg13 <- betareg(migration ~first_author_female + year + #subfield +
                      X1_gii + impact_factor + 
                      #subfield*first_author_female +     
                      impact_factor*first_author_female +
                      X1_gii*first_author_female, data = data)
...
model2_reg <- betareg(resilience ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
summary(model2_reg)

model2_reg2 <- betareg(food_security ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg3 <- betareg(vulnerability ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg4 <- betareg(adaptive_capacity ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg5 <- betareg(governance ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg6 <- betareg(local_knowledge ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg7 <- betareg(island_territories ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg8 <- betareg(climate_finance ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg9 <- betareg(rural_households ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg10 <- betareg(transformation_discourse ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg11 <- betareg(community ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg12 <- betareg(social_capital ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model2_reg13 <- betareg(migration ~last_author_female + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*last_author_female +     
                      impact_factor*last_author_female +
                      X2_gii*last_author_female, data = data)
...
model3_reg <- betareg(resilience ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
summary(model3_reg)

model3_reg2 <- betareg(food_security ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg3 <- betareg(vulnerability ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg4 <- betareg(adaptive_capacity ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg5 <- betareg(governance ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg6 <- betareg(local_knowledge ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg7 <- betareg(island_territories ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg8 <- betareg(climate_finance ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg9 <- betareg(rural_households ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg10 <- betareg(transformation_discourse ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg11 <- betareg(community ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg12 <- betareg(social_capital ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
model3_reg13 <- betareg(migration ~majority_female_binary + year + #subfield +
                      X2_gii + impact_factor + 
                      #subfield*majority_female_binary +     
                      impact_factor*majority_female_binary +
                      X2_gii*majority_female_binary, data = data)
...
### TWO-LEVEL INTERACTION ANALYSIS ###
# also do it per topic? or simply on climatejusticescore?
