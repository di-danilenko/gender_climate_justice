...
# match X1_country and X1_gii
# match X2_country and X2_gii
# match id with journal and journal impact
```
...
data1 <- read_csv("data_complete.csv")
...
for (i in seq_along(cj_topics)) {
  ...
  if (old_name %in% colnames(data1)) {
    colnames(data1)[colnames(data1) == old_name] <- new_name
  }
}
...
data1 <- merge(data1, gii_data, by.x="X1_country", by.y="country", all.x=T)
data1 <- data1 %>%
  rename(X1_gii = "gender inequality index")

data1 <- merge(data1, gii_data, by.x="X2_country", by.y="country", all.x=T)
data1 <- data1 %>%
